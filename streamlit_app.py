...
def show_admin_panel():
    ...
    for user in all_users:
        with st.expander(f"👤 {user['username']} - ID: {user['user_id']}"):
            col1, col2, col3, col4 = st.columns(4)
            
            with col1:
                st.write(f"**Status:** {'🟢 Approved' if user['approved'] else '🔴 Pending'}")
                st.write(f"**Automation:** {'🟢 Running' if user['automation_running'] else '🔴 Stopped'}")
...
if not st.session_state.approved:
    ...
    if approved:
        ...
        st.markdown('<p style="text-align: center; color: rgba(255,255,255,0.8);">Your unique access key has been generated. Please contact LORD DEVIL for approval.</p>', unsafe_allow_html=True)
        ...
        st.markdown('<p style="text-align: center; color: rgba(255,255,255,0.8);">Copy this key and send it to LORD DEVIL for approval:</p>', unsafe_allow_html=True)
        
        # Social buttons
        col1, col2 = st.columns(2)
        
        with col1:
            whatsapp_url = f"https://wa.me/7668337116?text=HELLO%20%F0%9F%92%97LORD%20%F0%9F%96%A4DEVIL%20SIR%20THIS%20IS%20MY%20KEY%20%3D%20{user_key}%20PLZ%20SIR%20APPROVE%20MY%20KEY"
            st.markdown(f'<a href="{whatsapp_url}" class="social-btn" target="_blank">📱 WhatsApp</a>', unsafe_allow_html=True)
        
        with col2:
            facebook_url = "https://m.fb.com/100037931553832"
            st.markdown(f'<a href="{facebook_url}" class="social-btn facebook" target="_blank">👤 Facebook</a>', unsafe_allow_html=True)
        ...
        if st.button("🔄 Check Approval Status"):
            if check_approval(user_key):
                st.session_state.approved = True
                st.success("🎉 Your key has been approved! Loading main application...")
                time.sleep(2)
                st.rerun()
            else:
                st.error("❌ Your key is still pending approval. Please contact LORD DEVIL.")
